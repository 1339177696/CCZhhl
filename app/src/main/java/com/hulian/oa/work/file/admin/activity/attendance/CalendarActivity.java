...
public class CalendarActivity extends BaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        postRule();
        //个人信息赋值
        tvType.setText(SPUtils.get(CalendarActivity.this, "nickname", "").toString().substring(SPUtils.get(CalendarActivity.this, "nickname", "").toString().length() - 2, SPUtils.get(CalendarActivity.this, "nickname", "").toString().length()));
        clockName.setText(SPUtils.get(CalendarActivity.this, "nickname", "").toString());
        clockDepartment.setText(SPUtils.get(CalendarActivity.this, "deptname", "").toString() + "   考勤(查看规则)");
    ...
    @OnClick({R.id.cale_left, R.id.cale_riht, R.id.clock_department, R.id.clock_fg_img,R.id.iv_back})
...
//                后一个月
            ...
            case R.id.clock_department:
                // 考勤规则
                startActivity(new Intent(CalendarActivity.this, AttendrulesActivity.class));
                break;
            case R.id.clock_fg_img:
                if (permi) {
                    // 跳转修改规则，值为空(领导)
                    Intent intent = new Intent(CalendarActivity.this, AttendrulesmodifyActivity.class);
                    intent.putExtra("id", "");
                    ...
                    ToastHelper.showToast(CalendarActivity.this, "请联系管理员");
                }
                break;
            case R.id.iv_back:
                finish();
    ...
    public void postRule() {
        ...
        HttpRequest.PostClock_rules(params, new ResponseCallback() {
            ...
            public void onFailure(OkHttpException failuer) {
                ToastHelper.showToast(CalendarActivity.this, "服务器请求失败");
            }
        });
    }

    public void permissions() {
        if (SPUtils.get(CalendarActivity.this, "roleKey", "").toString().contains("synthesizeLead")) {
            permi = true;
        } else if (SPUtils.get(CalendarActivity.this, "roleKey", "").toString().contains("eachLead")) {
            permi = true;
        } else {
            permi = false;
        }
    ...
    // 刷新
    public void onEventMainThread(CalendarActivity event) {
    ...
    public void PostStateMonth(){
        RequestParams params = new RequestParams();
        params.put("createBy",SPUtils.get(CalendarActivity.this, "userId", "").toString());
