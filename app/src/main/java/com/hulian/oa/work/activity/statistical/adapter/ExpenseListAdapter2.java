 ...
 * Describe:
 */
public class ExpenseListAdapter2 extends BaseExpandableListAdapter {
    //视图加载器
     ...
     */
    public ExpenseListAdapter2(Context context, List<Expen_Statis_Person_Bean> groupData, int expandedGroupLayout,
    ...
    private void bindChildView(View view, ExpenseStaBean data, List<ExpenseStaBean> peopleList, Expen_Statis_Person_Bean mDapart) {
        ...
        if (data.getCause().equals("null")){
            expens_name.setText(data.getCreateTime()+"");
        }else {
            expens_name.setText(data.getCreateTime()+"   (" + data.getCause() + ")");
        }
    }
