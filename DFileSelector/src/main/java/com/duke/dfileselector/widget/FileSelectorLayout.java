...
public class FileSelectorLayout extends RelativeLayout implements View.OnClickListener {
    ...
    public void refreshUI(String parentPath, List<FileItem> list) {
        if(parentPath.equals(fileHeadUrl)){
            headBackTextView.setVisibility(INVISIBLE);
        }
        else {
            headBackTextView.setVisibility(VISIBLE);
        }
    ...
    private void initData() {
        ...
        fileHeadUrl=rootFile.getAbsolutePath();
        Log.v("YYY",fileHeadUrl);
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.file_submit_text_view) {
            boolean flag=false;
            for(FileItem itemt:adapter.getList()){
                if(itemt.isChecked) {
                    flag = true;
            ...
            if(!flag){
                Toast.makeText(getContext(),"未选择任何附件文件",Toast.LENGTH_SHORT).show();
                return;
            }
